...
def clean_dish_header(line):
    ...
    # Clean formatting artifacts & underscores
    ...
    # Clean dish name aggressively to remove any leftover numbers, dots, or alcohol percentages
    name = re.sub(r'^\d+[\.\)]\s*', '', prefix).strip() # strip leading numbers e.g. "3.PIZZA"
    name = re.sub(r'[\s–\-]+\d+([.,]\d+)?\s*%?\s*alcool.*$', '', name, flags=re.IGNORECASE).strip() # strip alcohol %
    name = re.sub(r'\s+\d+(\s*,\s*\d+)*$', '', name).strip() # strip trailing allergen digits
    name = re.sub(r'[,._\-\s]+$', '', name).strip() # strip trailing punctuation
...
# High-quality natural English translator function
def translate_dish_name_clean(name_ro):
    n = name_ro.strip()
    
    # Exact mappings for known dishes
    exact_map = {
...
print(f"Master compiled {len(all_dishes)} 100% clean and accurately translated dish objects!")
