    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-slate-900/60 backdrop-blur-sm font-jakarta no-print">
        <motion.div
          ...
          className="bg-white w-full max-w-2xl rounded-t-3xl sm:rounded-3xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col border border-slate-200"
        >
          ...
          <div className="relative h-56 sm:h-64 w-full bg-slate-100 shrink-0">
            ...
            <div className="absolute bottom-4 left-4 right-4 text-white z-10 space-y-1">
              <div className="flex items-center gap-2 flex-wrap">
                {item.isThawed && (
                  <span className="bg-sky-500/90 text-white border border-sky-300 px-2.5 py-0.5 rounded-md text-[11px] font-bold flex items-center gap-1 shadow-xs">
                    <Snowflake className="w-3.5 h-3.5" />
                    <span>*Produs decongelat (ANPC Order 201/2022)</span>
                  </span>
                )}
                {item.isSpecialty && (
              ...
              <div className="flex items-center justify-between text-sm pt-1">
                <span className="bg-white/20 backdrop-blur-md px-3 py-0.5 rounded-lg font-bold">
                  Gramaj: {item.weight}
                </span>
          ...
          <div className="p-5 sm:p-6 overflow-y-auto space-y-6 flex-1 text-slate-900">
            ...
            <div className="space-y-2">
              <h3 className="text-sm font-extrabold text-[#C19B77] uppercase tracking-wider flex items-center gap-1.5">
                <span>📝</span>
                <span>{lang === 'ro' ? 'Lista Ingrediente' : lang === 'en' ? 'Ingredient List' : 'Összetevők'}</span>
              </h3>
            ...
            {/* ANPC Order 201/2022 Nutritional Values Table (Per Portion & Per 100g) */}
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-extrabold text-[#C19B77] uppercase tracking-wider flex items-center gap-1.5">
                  <Activity className="w-4 h-4 text-[#C19B77]" />
                  <span>{lang === 'ro' ? 'Valori Nutriționale (Conform ANPC Order 201/2022)' : 'Nutritional Values (ANPC Order 201/2022)'}</span>
                </h3>
              </div>
              ...
              <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-2xs">
                <table className="w-full text-left text-xs">
                  <thead className="bg-[#F8F6F2] border-b border-slate-200 text-slate-900 font-extrabold">
                    <tr>
                      <th className="py-2.5 px-3">Declaratie Nutrițională</th>
                      <th className="py-2.5 px-3 text-right">Per Portion ({item.weight})</th>
                      <th className="py-2.5 px-3 text-right bg-amber-100/50">Per 100g</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100 font-inter text-slate-700">
                    <tr>
                      <td className="py-2 px-3 font-semibold">Valoare Energetică (kcal / kJ)</td>
                    ...
                    <tr>
                      <td className="py-2 px-3 font-semibold">Grăsimi (g)</td>
                    ...
                    <tr>
                      <td className="py-2 px-3 pl-6 text-slate-500 text-[11px]">- din care acizi grași saturați (g)</td>
                    ...
                    <tr>
                      <td className="py-2 px-3 font-semibold">Glucide / Carbohidrați (g)</td>
                    ...
                    <tr>
                      <td className="py-2 px-3 pl-6 text-slate-500 text-[11px]">- din care zaharuri (g)</td>
                    ...
                    <tr>
                      <td className="py-2 px-3 font-semibold">Proteine (g)</td>
                    ...
                    <tr>
                      <td className="py-2 px-3 font-semibold">Sare / Sodium (g)</td>
            ...
            <div className="space-y-2">
              <h3 className="text-sm font-extrabold text-[#C19B77] uppercase tracking-wider flex items-center gap-1.5">
                ...
                <span>{lang === 'ro' ? 'Alergeni Declarați (Directiva EU 2000/13/CE)' : 'Declared Allergens (EU Directive 2000/13/CE)'}</span>
              </h3>
              ...
              <div className="flex flex-wrap gap-2">
                {item.allergens && item.allergens.length > 0 ? (
                  ...
                  <span className="text-xs text-emerald-700 font-bold bg-emerald-50 px-3 py-1.5 rounded-xl border border-emerald-200">
                    {lang === 'ro' ? '✓ Acest preparat nu conține alergeni declarați' : '✓ No declared allergens'}
                  </span>
                )}
              </div>
            </div>
