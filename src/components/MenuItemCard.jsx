...
import { Snowflake, Activity, ChevronDown, ChevronUp, AlertCircle, Sparkles } from 'lucide-react';
...
export default function MenuItemCard({ item, onSelectAllergen, lang = 'ro' }) {
  ...
  return (
    <div className="card-white rounded-2xl overflow-hidden flex flex-col justify-between transition-all duration-300 relative group">
      {/* Top Details & Pricing */}
      <div className="p-4 space-y-3 flex-1 flex flex-col justify-between">
        <div className="space-y-2">
          {/* Header Row: Title & Price Tag */}
          <div className="flex items-start justify-between gap-3">
            <h3 className="font-['Playfair_Display'] font-bold text-base md:text-lg text-[#1C1C1C] group-hover:text-[#C19B77] transition-colors leading-snug">
              {itemName}
            </h3>

            {/* Price Badge */}
            <div className="bg-[#F8F6F2] border border-[#C19B77]/30 px-3 py-1 rounded-xl shrink-0 text-right shadow-xs">
          ...
          <div className="flex items-center gap-2">
            <span className="inline-block bg-[#F8F6F2] text-[#7A7A7A] border border-[#E8E2D9] px-2 py-0.5 rounded-md text-[10px] font-mono font-semibold">
              {item.weight}
            </span>
        ...
        <div className="pt-2 border-t border-[#E8E2D9] flex items-center justify-between gap-2 text-xs">
              ...
              item.allergens.map((algId) => {
                ...
                return (
                  <button
                    ...
                    className="w-5 h-5 rounded-full bg-[#F8F6F2] hover:bg-[#C19B77] text-[#7A7A7A] hover:text-white border border-[#E8E2D9] text-[10px] font-bold flex items-center justify-center transition-all"
                    title={`${algId}. ${algName}`}
                  >
                    {algId}
                  </button>
          ...
          <button
            ...
            className="flex items-center gap-1 text-[11px] font-semibold text-[#C19B77] hover:text-[#A8805B] transition-colors shrink-0"
          >
            <Activity className="w-3.5 h-3.5" />
            <span>{lang === 'ro' ? '100g Nutrition' : '100g Info'}</span>
            {showNutrition ? <ChevronUp className="w-3 h-3" /> : <ChevronDown className="w-3 h-3" />}
          </button>
        </div>
      </div>
